...
我们研究一种典型的情况：function从一个lambda表达式构造，而lambda表达式只捕获了一个对象的引用（指针），在lambda表达式里，我们调用了对象的一个虚函数。代码如下
```c++
class struct Clss {
    int c = 0;
...
我们采用了GCC 9.2和VS2019对样例进行编译，目标平台设定为x86-amd64。我们发现其生成的代码几乎是完全一样的。所以，我们只展示GCC 9.2的反汇编结果。现在我们把两个函数反汇编分别摘抄如下
